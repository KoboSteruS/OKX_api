...
Приложение генерирует **актуальную временную метку в формате ISO 8601** в момент запроса. Используйте полученную подпись и временную метку **немедленно** для запроса к OKX API.

**Формат временной метки**: `2025-07-25T12:30:45Z` (без микросекунд, с Z вместо +00:00)
...
```json
{
  ...
  "ok_access_timestamp": "2025-07-25T12:30:45Z",
  "method": "POST",
  "path": "/api/v5/trade/order"
}
```
